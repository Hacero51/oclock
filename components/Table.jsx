      ...
      <div className="hidden md:block overflow-y-auto rounded-2xl border border-gray-200 shadow-sm bg-white ring-1 ring-gray-100">
        <table className="w-full text-sm text-left">
          <thead className="bg-blue-500 border-b border-white-900">
            <tr>
              {columnas.map((col) => (
                <th key={col} className="px-6 py-4 text-xs font-semibold text-white-500 uppercase tracking-wider">
                  {col}
                </th>
              ))}
          ...
          <tbody className="divide-y divide-gray-900">
            {datos.length > 0 ? (
              datos.map((fila, i) => (
                ...
                >
                  {columnas.map((col) => (
                    <td key={col} className="px-6 py-4 text-gray-600 font-medium group-hover:text-gray-900 transition-colors">
                      {fila[col]}
                    </td>
                  ))}
                  {onRowClick && (
              ...
              <tr>
                <td colSpan={columnas.length + (onRowClick ? 1 : 0)} className="px-6 py-12 text-center text-gray-400 italic">
          ...
          datos.map((fila, i) => (
            ...
            >
              ...
              <div className="space-y-3">
                {columnas.map((col) => (
                  <div key={col} className="flex justify-between items-start border-b border-gray-50 last:border-0 pb-2 last:pb-0">
                    <span className="text-xs font-semibold text-gray-400 uppercase tracking-wide mt-0.5">{col}</span>
