...
import { Clock, X, Settings, AlertTriangle, CalendarDays, Plus, Trash2, CalendarOff } from 'lucide-react';
...
export default function UpdateHorariosForm({ data, onClose }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (data?.Oid) {
          const resDetail = await fetch(`/api/horarios/${data.Oid}`);
          if (resDetail.ok) {
        ...
        if (dataDetails) {
          ...
          if (dataDetails.turnos && Array.isArray(dataDetails.turnos)) {
            const mappedTurnos = dataDetails.turnos.map(t => ({
              Shift: t.Shift, // This refers to the Shift OID
              Day: t.Day,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`/api/horarios/${data.Oid}`, {
        method: 'PUT',
    ...
    <div className="w-full h-full flex flex-col bg-white font-sans">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-white sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900">Actualizar Horario <span className="text-gray-400 font-normal">| {headerName}</span></h2>
            <p className="text-xs text-gray-500">Modifique los detalles de la jornada laboral</p>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
        {loading ? (
          <div className="flex items-center justify-center h-full text-gray-400">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mr-2"></div> Cargando...
          </div>
        ) : (
      ...
      <div className="p-4 bg-white border-t border-gray-200 flex justify-end gap-3 sticky bottom-0 z-10">
        ...
        <button
          ...
          className="px-6 py-2.5 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors shadow-sm hover:shadow focus:ring-4 focus:ring-blue-100"
        >
          Guardar Cambios
        </button>
      </div>
